...
import modeller.lda.data.text.BasicTokenizer;
import modeller.lda.data.text.LowerCase;
import modeller.lda.data.text.PorterStemmer;
import modeller.lda.data.text.StopWordRemoval;
...
//        }
    ...
    private List<TupleTwo<Integer, TupleTwo<List<String>, String>>> map1(List<FInput> batch1){

        int m = batch1.size();

        List<TupleTwo<Integer, TupleTwo<List<String>, String>>> result = new ArrayList<>();
        for(int i=0; i < m; ++i){
            FInput blogPost =   batch1.get(i);
         //   String doc = batch1.get(i);
            String doc = blogPost.getPost();
    ...
    public LdaResult fit(List<FInput> docs) {
        ...
        for(int docIndex = 0; docIndex < size; ++docIndex){
            ...
            document.setBlogPost(docs.get(docIndex));
//document.set
        ...
        for(int sweepIndex = 0; sweepIndex < maxSweepCount; ++sweepIndex){
            ...
            for(int docIndex=0; docIndex < size; ++docIndex){
                Doc<B> currentDoc = documents.get(docIndex);
